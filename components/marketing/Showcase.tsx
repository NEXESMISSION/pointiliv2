...
export function demoQr(color = "#0c0c14") {
  return QRCode.toString(`${siteUrl()}/how-it-works`, { type: "svg", margin: 0, errorCorrectionLevel: "M", color: { dark: color, light: "#00000000" } });
}
...
export function HeroShowcase({ qr }: { qr: string }) {
  return (
    <div role="img" aria-label="A café shows its Pointili QR at the counter; the customer's phone shows a stamp added to their loyalty card" className="relative grid grid-cols-[1fr_1.12fr] items-center">
      {/* Counter QR */}
      <div className="relative -mr-5 -rotate-3 rounded-[1.25rem] border border-line bg-white p-3 shadow-lift sm:-mr-8 sm:p-5">
        <div className="flex items-center gap-2">
          ...
          <span className="min-w-0 text-left leading-tight">
            ...
            <span className="block truncate text-[10px] text-muted sm:text-xs">Scan for a stamp</span>
          </span>
        </div>
        ...
        <p className="mt-1.5 text-center text-[10px] text-muted sm:text-xs">New code in 0:42</p>
      </div>
      ...
      <div className="relative z-10 rounded-[2rem] border border-line bg-white p-1.5 shadow-lift sm:rounded-[2.5rem] sm:p-2">
        <div className="relative overflow-hidden rounded-[1.6rem] bg-canvas px-2.5 pb-3 pt-7 sm:rounded-[2rem] sm:px-4 sm:pb-5 sm:pt-10">
          ...
          <div className="flex animate-pop items-center gap-2 rounded-xl border border-line bg-white p-1.5 text-left shadow-card sm:p-2.5" style={{ animationDelay: "500ms" }}>
            ...
            <span className="min-w-0 flex-1 truncate text-[10px] font-semibold text-ink sm:text-[13px]">Stamp added</span>
            <span className="text-[9px] text-muted sm:text-xs">now</span>
          </div>

          <div className="mt-2 rounded-2xl p-2.5 text-left text-white sm:mt-3 sm:p-4" style={{ background: "linear-gradient(150deg, #7547EE 0%, #5029C5 100%)" }}>
            <div className="flex items-center gap-2">
            ...
            <div className="mt-2.5 flex items-center gap-1.5 rounded-lg bg-white/12 px-2 py-1.5 text-[9px] font-medium sm:mt-4 sm:rounded-xl sm:px-3 sm:py-2 sm:text-xs">
              ...
              <span className="min-w-0 flex-1 truncate">Free coffee</span>
              <span className="shrink-0 text-white/70">3 to go</span>
            </div>
          </div>

          <div className="mt-2 flex items-center gap-2 rounded-xl border border-line bg-white p-2 text-left sm:mt-3 sm:p-3">
...
export function StepVisual({ step, qr }: { step: "qr" | "scan" | "stamp" | "reward"; qr: string }) {
  return (
      ...
      <div className="relative">
        ...
        {step === "stamp" && (
          <div className="w-52 rounded-2xl p-3.5 text-white shadow-lift" style={{ background: "linear-gradient(150deg, #7547EE 0%, #5029C5 100%)" }}>
            ...
            <span className="absolute -right-3 -top-3 rounded-full bg-success-500 px-2.5 py-1 text-xs font-bold text-white shadow-lift">+1</span>
          </div>
        )}
        ...
        {step === "reward" && (
          <div className="flex w-52 items-center gap-3 rounded-2xl border border-line bg-white p-3.5 shadow-lift">
            ...
            <span className="min-w-0 flex-1 text-left">
              <span className="block text-sm font-semibold text-ink">Free coffee</span>
